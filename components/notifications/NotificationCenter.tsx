...
export const NotificationCenter: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-4 text-left max-w-3xl mx-auto w-full">
      {/* Header section - Balanced & Minimal */}
      <div className="flex items-center justify-between gap-3 pb-3 border-b border-black dark:border-white">
        <div className="flex items-center gap-2.5">
          <h2 className="text-lg sm:text-xl font-bold uppercase tracking-wider text-black dark:text-white">
            Inbox
          </h2>
          {unreadCount > 0 && (
            <span className="px-2 py-0.5 border border-black dark:border-white bg-black text-white dark:bg-white dark:text-black text-[10px] font-mono font-bold uppercase tracking-wider">
              {unreadCount} NEW
            </span>
          )}
        </div>
        ...
        {notifications.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 border border-black dark:border-white text-xs font-bold uppercase tracking-wider hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors cursor-pointer shrink-0"
          >
        ...
        {(['all', 'classes', 'events', 'homework', 'deadlines', 'carry', 'system'] as const).map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold uppercase tracking-wider transition-all border whitespace-nowrap cursor-pointer shrink-0 ${
                isSelected
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((notif) => (
              <motion.div
                ...
                className={`group relative flex items-start justify-between gap-3.5 p-3.5 sm:p-4 border transition-all cursor-pointer ${
                  notif.read
                ...
                <div className="flex items-start gap-3.5 w-full min-w-0">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="font-mono text-[11px] text-black/50 dark:text-white/50 shrink-0">
                    ...
                    <p className="text-xs sm:text-sm text-black/75 dark:text-white/75 leading-relaxed font-medium">
                      {notif.message}
                    </p>
                  </div>
                </div>
