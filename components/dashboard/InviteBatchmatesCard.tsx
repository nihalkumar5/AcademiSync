...
👉 Open Intersemester App → Tap *Connect Batch* → Enter Code: *${batchCode}*`;
  ...
  return (
    <>
      <div className="relative bg-[#F4F4F4] dark:bg-[#121317] p-4 sm:p-5 border border-[#D9D9D6] dark:border-white/[0.08] dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] flex items-center justify-between overflow-hidden min-h-[140px] rounded-none">
        <div className="flex flex-col gap-3 max-w-[65%] sm:max-w-[70%] relative z-10">
          <div className="flex flex-col gap-1">
            <h3 className="text-[16px] font-bold text-[#111111] dark:text-[#F4F4F6] tracking-tight flex items-center gap-2">
              {isSynced ? (
                'Share Batch Code'
              ) : (
                <>
                  <Users className="w-4 h-4 text-[#111111] dark:text-[#F4F4F6]" />
                  <span>Connect with Your Batch</span>
                </>
              )}
            </h3>
            <p className="text-[12px] text-[#6F6F6F] dark:text-[#94A3B8] leading-tight pr-2">
              {isSynced
            ...
            {isSynced && sixDigitCode && (
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-[10px] font-bold text-[#6F6F6F] dark:text-[#A1A1AA] uppercase tracking-wider">
                  Batch Code:
                </span>
                <span className="text-[12px] font-mono font-bold tracking-[1.5px] text-[#111111] dark:text-[#F4F4F6] bg-black/5 dark:bg-white/[0.08] px-2 py-0.5 rounded border border-black/10 dark:border-white/10 select-all">
                  {sixDigitCode}
                </span>
              </div>
            )}
          ...
          {isSynced ? (
            <button
              ...
              className="self-start mt-1 px-4 py-2 bg-[#111111] dark:bg-white text-[#FFFFFF] dark:text-[#090A0C] text-[11px] font-bold tracking-[1px] uppercase transition-opacity hover:opacity-90 rounded-none cursor-pointer"
            >
              Share Code
            </button>
          ) : (
            <button
              ...
              className="self-start mt-1 px-4 py-2 bg-[#111111] dark:bg-white text-[#FFFFFF] dark:text-[#090A0C] text-[11px] font-bold tracking-[1px] uppercase transition-opacity hover:opacity-90 rounded-none cursor-pointer flex items-center gap-1.5"
            >
        ...
        <div className="absolute right-0 bottom-0 z-0 flex items-end justify-end h-[100%] pr-0 pointer-events-none">
          ...
          <div className="hidden dark:block absolute right-3 bottom-2 w-[135px] sm:w-[155px] h-[115px] sm:h-[130px] rounded-none bg-white/90 -z-10" />
