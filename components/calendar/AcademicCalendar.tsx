      ...
      return (
        <div
          ...
          className={clsx(
            ...
            isSelected && 'bg-[#111111] text-[#FFFFFF] dark:bg-[#FFFFFF] dark:text-[#111111]',
            !isSelected && isToday && 'text-[#111111] dark:text-[#FFFFFF] font-bold'
          )}
        >
          ...
          {dayItems.length > 0 && (
            <span className={clsx("w-1 h-1 rounded-full absolute bottom-2", isSelected ? 'bg-white dark:bg-black' : 'bg-[#111111] dark:bg-[#FFFFFF]')} />
          )}
        </div>
      );
    });
        ...
        <div className="flex flex-col gap-4">
          {upcomingItems.length === 0 ? (
            ...
            upcomingItems.map((item, idx) => {
              ...
              return (
                <div key={idx} className="border border-[#D8D8D8] dark:border-[#333333] bg-[#FFFFFF] dark:bg-[#111111] p-5 rounded-none flex items-center justify-between group cursor-pointer hover:border-[#111111] dark:hover:border-[#FFFFFF] transition-colors">
                  <div>
                    <p className="text-[12px] font-mono text-[#A0A0A0] mb-2">{dayStr} {monthStr}</p>
                    <h4 className="text-[12px] font-bold tracking-[1px] uppercase text-[#111111] dark:text-[#FFFFFF] mb-1">
                      {item.type.replace('-', ' ')}
                    </h4>
                    <p className="text-[15px] font-medium text-[#111111] dark:text-[#FFFFFF] leading-snug">
                  ...
                  <ArrowRight className="w-5 h-5 text-[#D8D8D8] dark:text-[#333333] group-hover:text-[#111111] dark:group-hover:text-[#FFFFFF] transition-colors" />
                </div>
              );
            })
          )}
             ...
             {(itemsByDate.get(selectedDate) || []).length === 0 ? (
                ...
                (itemsByDate.get(selectedDate) || []).map((item, idx) => (
                  <div key={idx} className="flex gap-4 border-b border-[#D8D8D8] dark:border-[#333333] pb-3 last:border-0">
                    ...
                    <div>
                      <p className="text-[13px] font-medium text-[#111111] dark:text-[#FFFFFF]">
                        {item.title} {item.subject && `• ${item.subject}`}
                      </p>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 sm:p-0 bg-black/40 backdrop-blur-sm">
           <div className="bg-[#FFFFFF] dark:bg-[#111111] w-full max-w-sm border border-[#111111] dark:border-[#FFFFFF] p-6 shadow-2xl relative">
              ...
              <div className="flex flex-col gap-4">
                 ...
                 <button onClick={handleSaveEvent} className="w-full h-10 bg-[#111111] text-[#FFFFFF] dark:bg-[#FFFFFF] dark:text-[#111111] text-[13px] font-bold mt-4">
                    Save Event
                 </button>
              </div>
           </div>
